...
namespace v2rayN.Handler
{
    ...
    class V2rayHandler
    {
        ...
        public int LoadV2rayConfigString(Config config, List<int> _selecteds)
        {
            ...
            {
                ShowMsg(false, msg);
                pid = V2rayStartNew(configStr, config);
                //V2rayRestart();
                // start with -config
            }
            return pid;
        ...
        /// V2ray重启
        /// </summary>
        private void V2rayRestart(Config config)
        {
            V2rayStop();
            V2rayStart(config);
        }
        ...
        private string V2rayFindexe(string fname) {
            //查找v2ray文件是否存在
            ...
            foreach (string name in lstV2ray)
            {
                if( fname == name)
                {
                    ...
                    fname = vName;
                    //ShowMsg(false, "Searching Program:" + vName);
                    if (File.Exists(vName))
                    {
                        ShowMsg(false, "Use Program path:" + vName);
                        fileName = vName;
                        break;
                    }
                }
            ...
            if (Utils.IsNullOrEmpty(fileName))
            {
                ...
                string msg = "Can't find the programe name of :" + fname;
                ShowMsg(false, msg);
            }
            return fileName;
        }
        ...
        /// V2ray启动
        /// </summary>
        private void V2rayStart(Config config)
        {
            ...
            try
            {
                ...
                string fileName = V2rayFindexe(fname);
                if (fileName == "") return;
        ...
        /// </summary>
        private int V2rayStartNew(string configStr, Config config )
        {
            ShowMsg(false, string.Format(UIRes.I18N("StartService"), DateTime.Now.ToString()));
