...
namespace v2rayN.Handler
{
    ...
    class V2rayConfigHandler
    {
        ...
        private static string SampleTrojanClient = Global.SampleTrojanClient;
        ...
        public static int GenerateClientConfig(Config config, string fileName, bool blExport, out string msg)
        {
            try
            {
                ...
                {
                    return GenerateTrojanConfig(config, fileName, out msg);
                }
        ...
        /// <param name="msg"></param>
        /// <returns></returns>
        public static int GenerateTrojanConfig(Config config, string fileName, out string msg)
        {
            msg = string.Empty;
            try
            {
                ...
                string embedText = Utils.GetEmbedText(SampleTrojanClient);
                if (Utils.IsNullOrEmpty(embedText))
                {
                    msg = UIRes.I18N("FailedGetDefaultConfiguration");
                    return -1;
                }
                TrojanConfig trojanConfig = Utils.FromJson<TrojanConfig>(embedText);
                if (trojanConfig == null)
                {
                    msg = UIRes.I18N("FailedGenDefaultConfiguration");
                    return -1;
                }
                trojanConfig.remote_addr = config.address();
                trojanConfig.password[0] = config.id();
                trojanConfig.remote_port = config.port();
                trojanConfig.local_port = config.inbound[0].localPort;
                Utils.ToJsonFile(trojanConfig, fileName);
                msg = string.Format(UIRes.I18N("SuccessfulConfiguration"), config.getSummary());
            }
            catch
            {
